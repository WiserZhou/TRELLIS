...
def find_ckpt(cfg):
    # Load checkpoint
    # Sets the checkpoint to load based on configuration
    # 'latest': finds the most recent checkpoint
    # 'none': doesn't load any checkpoint
    # specific step number: loads that specific checkpoint
    cfg['load_ckpt'] = None
    if cfg.load_dir != '':
        if cfg.ckpt == 'latest':
...
def setup_rng(rank):
    # Set up random number generators with the given seed
    # This ensures reproducibility across different runs
    # The seed is based on the process rank to ensure different processes have different randomness
...
def get_model_summary(model):
    # Generate a detailed summary of the model parameters
    # Counts total parameters and trainable parameters
    # Returns a formatted string with parameter details
...
def main(local_rank, cfg):
    ...
    # Sets up distributed training, dataset, model, and starts training
...
if __name__ == '__main__':
    ...
    if cfg.auto_retry == 0:
        ...
        # Try training with auto-retry on errors
