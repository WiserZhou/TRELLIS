...
BLENDER_INSTALLATION_PATH = '/mnt/pfs/users/yangyunhan/yufan/data/blender'
...
def _render(file_path, sha256, output_dir):
    ...
    if os.path.exists(os.path.join(output_folder, 'transforms.json')):
        os.remove(os.path.join(output_folder, 'transforms.json'))

    # 检查输出文件夹中是否存在保存的 ply 文件
    ply_file_path = os.path.join(output_folder, 'mesh.ply')

    if os.path.exists(ply_file_path):
        print(f"Rendered {sha256} successfully.")
        return {'sha256': sha256, 'saved': True}
    else:
        print(f"Failed to render {sha256}.")
        return {'sha256': sha256, 'saved': False}
...
if __name__ == '__main__':
    ...
    # process objects
    func = partial(_render, output_dir=opt.output_dir)
