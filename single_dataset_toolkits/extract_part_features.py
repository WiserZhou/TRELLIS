...
def extract_part_features(render_info, output_dir, feature_name, dinov2_model, batch_size, image_size=518, transform=None):
    """Extract features for individual parts of the 3D model
    
    Args:
        render_info: Dictionary containing part render information
        output_dir: Directory to save features
        feature_name: Name of the feature type
        dinov2_model: Model for feature extraction
        batch_size: Batch size for processing
        image_size: Target image size for processing
        transform: Image transformation function
    """
    if "parts" not in render_info:
        print("No parts information found in render_info")
        return
        
    print(f"Found {len(render_info['parts'])} parts in render_info")
    
    # Process each part
    for part_idx, part_info in enumerate(render_info["parts"]):
        part_name = part_info["name"]
        part_index = part_info["index"]
        
        print(f"Processing part {part_index}: {part_name}")
        
        # Prepare part-specific render information
        part_render_info = {
            "image_paths": part_info["image_paths"],
            "camera_params": part_info["camera_params"],
            "normalization": part_info["normalization"]
        }
        
        try:
            # Load and process image data for this part
            data = load_image_data(part_render_info, image_size, transform)
            
            if not data:
                print(f"No valid data found for part {part_name}")
                continue
                
            print(f"Loaded {len(data)} valid images for part {part_name}")
                
            # Extract features in batches
            features_list = extract_features_batch(data, batch_size, dinov2_model)
            
            # Save part features
            all_features = torch.cat(features_list, dim=0)
            part_save_path = os.path.join(output_dir, 'part_features', feature_name, f'part_{part_index:04d}_{part_name}.pt')
            torch.save(all_features, part_save_path)
            print(f"Saved features for part {part_name} to {part_save_path}, tensor shape: {all_features.shape}")
            
            # Extract and save feature statistics for the part
            # Average features across all views to get a single representation for the part
            avg_features = torch.mean(all_features, dim=0).cpu().numpy().astype(np.float16)
            
            # Save the part features in a compact format
            part_features_path = os.path.join(output_dir, 'part_features', feature_name, f'part_{part_index:04d}_{part_name}_features.npz')
            np.savez_compressed(part_features_path, features=avg_features)
            print(f"Saved average part features to {part_features_path}, shape: {avg_features.shape}")
            
        except Exception as e:
            print(f"Error processing part {part_name}: {e}")
            traceback.print_exc()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--render_info', type=str, default="/mnt/pfs/users/yangyunhan/yufan/data/render_part_images/render_info.json",
                        help='Path to render_info.json file')
    parser.add_argument('--model', type=str, default='dinov2_vitl14_reg',
                        help='Feature extraction model')
    parser.add_argument('--batch_size', type=int, default=16,
                        help='Number of images to process in one batch')
    parser.add_argument('--process_whole', action='store_true',
                        help='Process whole model in addition to parts')
    parser.add_argument('--process_parts', action='store_true', default=True,
                        help='Process individual parts')
    opt = parser.parse_args()
    opt = edict(vars(opt))

    # Setup output directories for features
    feature_name = opt.model
    os.makedirs(os.path.join(opt.output_dir, 'part_features', feature_name), exist_ok=True)
    ...
    if opt.process_whole and "whole_model" in render_info and render_info["whole_model"]["image_paths"]:
        first_image_path = render_info["whole_model"]["image_paths"][0]
        model_id = os.path.basename(os.path.dirname(first_image_path))
        
        print(f"Processing whole model: {model_id}")
        
        try:
            # Setup whole model render info
            whole_render_info = {
                "image_paths": render_info["whole_model"]["image_paths"],
                "camera_params": render_info["whole_model"]["camera_params"],
                "normalization": render_info["whole_model"]["normalization"]
            }
            
            # Process whole model (existing implementation)
            data = load_image_data(whole_render_info, image_size, transform)
            
            if not data:
                print("No valid data found for whole model")
            else:
                print(f"Loaded {len(data)} valid images for whole model")
                ...
                if positions is not None:
                    ...
                    projected_path = os.path.join(opt.output_dir, 'part_features', feature_name, f'{model_id}_projected.npz')
                    np.savez_compressed(projected_path, **pack)
                    print(f"Saved projected features to {projected_path}, shape: {avg_features.shape}")
        except Exception as e:
            print(f"Error during whole model processing: {e}")
            traceback.print_exc()
    
    # Process parts if requested
    if opt.process_parts:
        extract_part_features(render_info, opt.output_dir, feature_name, dinov2_model, opt.batch_size, image_size, transform)
